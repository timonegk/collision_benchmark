 ...
 */
#include <collision_benchmark/benchmark_ik_solver.h>
...
std::string BenchmarkIKSolver::COLLISION_OBJECT_NAME = "reach_object";

BenchmarkIKSolver::BenchmarkIKSolver(moveit::core::RobotModelConstPtr model, const std::string& planning_group,
...
std::vector<std::vector<double>> BenchmarkIKSolver::solveIK(const Eigen::Isometry3d& target,
                                                         const std::map<std::string, double>& seed) const
{
  ...
  if (state.setFromIK(jmg_, target, 0.0,
                      std::bind(&BenchmarkIKSolver::isIKSolutionValid, this, std::placeholders::_1, std::placeholders::_2,
...
bool BenchmarkIKSolver::isIKSolutionValid(moveit::core::RobotState* state, const moveit::core::JointModelGroup* jmg,
...
std::vector<std::string> BenchmarkIKSolver::getJointNames() const
{
  return jmg_->getActiveJointModelNames();
}

void BenchmarkIKSolver::setHoleAxis(const tf2::Vector3 hole_axis) {
  hole_axis_ = hole_axis;
}

void BenchmarkIKSolver::setHolePosition(const tf2::Vector3 hole_position)
{
...
void BenchmarkIKSolver::addCollisionMesh(const std::string& collision_mesh_filename,
...
void BenchmarkIKSolver::setTouchLinks(const std::vector<std::string>& touch_links)
{
  scene_->getAllowedCollisionMatrixNonConst().setEntry(COLLISION_OBJECT_NAME, touch_links, true);
}

std::string BenchmarkIKSolver::getKinematicBaseFrame() const
{
  return jmg_->getSolverInstance()->getBaseFrame();
}

reach::IKSolver::ConstPtr BenchmarkIKSolverFactory::create(const YAML::Node& config) const
{
  ...
  auto ik_solver = std::make_shared<BenchmarkIKSolver>(model, planning_group, dist_threshold);
